-- Draft format setting: snake (default) or classic.
ALTER TABLE public.leagues
  ADD COLUMN IF NOT EXISTS draft_type TEXT NOT NULL DEFAULT 'snake'
  CHECK (draft_type IN ('snake', 'classic'));

-- Ensure league duplication preserves the draft type.
...
BEGIN
  ...
  VALUES (
    v_name,
    COALESCE(v_source.draft_type, 'snake'),
    v_source.num_teams,
    v_source.num_rounds,
...
-- Make mock initialization honor league draft_type when there is no live board.
  ...
  ELSE
    ...
    FOR v_round IN 1..v_rounds LOOP
      FOR v_idx IN 1..v_team_count LOOP
        IF COALESCE(v_draft_type, 'snake') = 'snake' AND v_round % 2 = 0 THEN
