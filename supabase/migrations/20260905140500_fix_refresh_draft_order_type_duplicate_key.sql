-- Fix duplicate-key collisions when switching draft type mid-draft.
-- Keep slot identity (original_team_id) unchanged; only reassign current owner.
...
BEGIN
  ...
  LOOP
    ...
    -- Only reassign current owner to avoid violating unique(league_id, round, original_team_id, year).
